...
// Acción asíncrona para actualizar datos empresariales
export const actualizarDatosEmpresa = createAsyncThunk(
  "autenticacion/actualizarDatosEmpresa",
  async ({ id, datos }: { id: number; datos: ActualizarDatosEmpresaRequest }) => {
    await repoCliente.actualizarDatosEmpresa(id, datos);
    // Si la actualización es exitosa, obtenemos los datos actualizados
    return await repoCliente.obtenerPorId(id);
  }
);
...
const autenticacionSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(actualizarCliente.fulfilled, (state, action) => {
        ...
        if (action.payload) {
          state.usuario = {
            ...state.usuario!,
            ...action.payload,
            // Actualizar nombre_completo según tipo de documento
            nombre_completo: action.payload.tipo_documento === 'RUC' 
              ? action.payload.razon_social 
              : `${action.payload.nombres} ${action.payload.apellidos}`
          };
        }
      })
      .addCase(actualizarCliente.rejected, (state, action) => {
      ...
      .addCase(actualizarDatosEmpresa.fulfilled, (state, action) => {
        state.cargando = false;
        if (action.payload) {
          state.usuario = {
            ...state.usuario!,
            ...action.payload,
            // Actualizar nombre_completo para empresas
            nombre_completo: action.payload.razon_social
          };
        }
      })
      .addCase(actualizarDatosEmpresa.rejected, (state, action) => {
        state.cargando = false;
        state.error = action.error.message || "Error al actualizar datos de empresa";
      })
