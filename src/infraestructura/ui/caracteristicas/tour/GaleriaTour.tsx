...
const GaleriaTour = ({ idTipoTour, nombreTour, imagenPrincipal }: GaleriaTourProps) => {
  ...
  // Cargar imágenes de la galería y datos del tipo de tour cuando cambia el ID
  useEffect(() => {
    if (idTipoTour) {
      // Cargar galería de imágenes
      dispatch(listarGaleriasTourPorTipoTour(idTipoTour));
      
      // Solo cargar el tipo de tour si no está ya cargado o si es diferente
      if (!tipoTourActual || tipoTourActual.id_tipo_tour !== idTipoTour) {
        dispatch(obtenerTipoTourPorId(idTipoTour));
      }
    }
  }, [dispatch, idTipoTour, tipoTourActual]);
  ...
  const obtenerImagenPrincipal = (): string => {
    ...
    if (galeriasTour.length > 0 && galeriasTour[0].url_imagen) {
  ...
  const prepararImagenes = (): string[] => {
    ...
    const imagenesGaleria = galeriasTour.map(item => item.url_imagen || '').filter(url => url !== '');
